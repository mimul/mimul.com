...
이 이상 점수가 어떤 분포를 따르는지는 샘플 크기 $N$에 따라 달라진다. $N$이 충분히 크면(대략 100 이상) 이상 점수는 자유도 1인 카이제곱분포에 근사한다. $N$이 작으면 정규분포의 평균과 분산을 모두 표본에서 추정한 데서 오는 불확실성이 무시할 수 없을 만큼 커지는데, 이때 표준화된 편차는 자유도 $N-1$인 스튜던트 t분포를 따르고, t분포를 따르는 확률변수의 제곱은 다음 항등식에 따라 F분포를 따른다.

$$T \sim t_{N-1} \implies T^2 \sim F_{1,\,N-1}$$

즉 이상 점수도 $N$이 작을 때는 자유도 $(1, N-1)$인 F분포를 따르게 된다. 실무에서는 대개 샘플 크기가 충분히 크므로, 카이제곱분포 기준으로 임계값을 구하는 코드를 보자.
...
목표 오탐률(false positive rate)은 정상 데이터를 이상으로 잘못 판정할 확률의 상한이다. 이 값을 낮게 잡을수록 임계값이 높아져 오탐은 줄지만 실제 이상을 놓칠 위험은 늘어난다. 이 임계값(9.00)은 목표 오탐률과 자유도로만 정해지므로 어떤 1변량 변수에 적용하든 같은 값이다. `temp2`에 이 임계값을 적용하면, 추론 데이터 510건 중 10건이 이를 넘어 이상으로 판정된다.
