...
![langgraph state](./langgraph_state_three.png)
...
![langgraph tool calling state](./langgraph_tool_calling_state_three.png)
...
- `tool_node = ToolNode(tools)`에서 ToolNode는 LangGraph에서 제공하는 내장 노드이다. 이 노드는 그래프 상태(state)에서 `tool_calls`를 가진 AIMessage를 받으면 해당 툴을 실행하고 그 결과를 ToolMessage 객체로 생성한다. 이 노드를 사용하려면 state에 messages 키가 반드시 있어야 하고, 가장 마지막 메시지가 `tool_calls` 속성을 가진 AIMessage여야 한다.
