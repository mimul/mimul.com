...
description: I/O Multiplexing 대두된 배경, select, poll, epoll, kqueue에 대해 설명.
author: Mimul
...
- 다만, 이벤트 드리븐(event-driven) 패러다임으로 설계하면 작업 중 일부 코드가 블로킹되면 전체 이벤트 루프가 멈춰버릴 위험이 있으므로 주의해야 한다. 
- 라이브러리(예: libevent) 등을 이용하면 OS별로 다른 I/O Multiplexing 방식을 추상화해서 활용할 수 있다.
