---
title: 캐시 설계에서 빠지기 쉬운 Penetration, Breakdown, Avalanche 문제 해결
description: 분산 시스템에서 자주 발생하는 Cache Penetration, Cache Breakdown, Cache Avalanche 문제의 원인과 블룸 필터, 분산 락, 만료 시간 분산 등 실전 해결책을 코드와 함께 설명합니다.
author: Mimul
...
tags:
  ...
  - redis
---
분산 시스템의 설계에서 캐시는 성능 향상을 위해 매우 중요한 요소지만, 캐시를 설계함에 있어서 여러 문제점들을 고려하지 않는다면 오히려 성능상의 단점이나 가용성 문제에 직면할 수 있다. 백엔드 개발자나 분산 시스템을 설계하는 엔지니어라면 반드시 알아야 할 세 가지 대표적인 캐시 문제 — Cache Penetration, Cache Breakdown, Cache Avalanche — 의 원인과 해결 방법을 정리해 본다.
...
아래는 Cache Penetration 현상에 대한 두 가지 해결책에 대해 기술해 본다.

**해결책 1) Empty 결과 캐싱**

쿼리가 반환하는 데이터가 비어 있더라도 결과를 캐시하여 이 문제를 해결하게 된다.

![empty 캐싱](./cache_penetration_ans1.png)
...
데이터를 검색하기 전에 Bloom 필터를 사용하여 DB에 데이터가 없는지 여부를 결정한다. 존재하지 않는 데이터에 대해서는 조기 리턴을 실시해, 캐시나 데이터베이스에의 액세스가 발생하지 않게 한다.

![bloomfilter](./cache_penetration_ans2.png)

이 경우는 존재할 수 있는 데이터의 경우 먼저 캐시를 검색하고 데이터가 없으면 DB에 액세스하여 결과를 fetch 한다. 검색 키의 반복률이 낮고 검색 키의 종류가 매우 많은 경우에는 블룸 필터를 사용하는 것이 효과적이다. 블룸 필터를 알려면 [Bloom Filter 소개](https://www.mimul.com/blog/bloom-filter/)라는 글을 보면 좋을 것 같다.
...
![검색키잠금](./cache_breakdown_ans1.png)

이 방법은 구현이 단순하지만, 락 경쟁으로 인한 대기 시간이 발생할 수 있어 고트래픽 환경에서는 병목이 될 수 있다.
...
**해결책 2) 논리적 만료(Logical Expiry) 활용**
...
Cache Penetration은 존재하지 않는 키에 대한 반복 조회로 DB가 직접 타격을 받는 문제로, 빈 결과 캐싱이나 블룸 필터로 대응한다. Cache Breakdown은 인기 키의 만료 순간에 동시 요청이 몰리는 문제로, 분산 락이나 논리적 만료로 DB 부하를 분산한다. Cache Avalanche는 대규모 캐시가 동시에 만료되거나 캐시 서버 자체가 다운될 때 발생하며, 만료 시간 분산, 클러스터 구성, Rate Limit을 조합해 방어한다. 세 가지 문제는 각각 발생 맥락이 다르므로 시스템의 트래픽 패턴과 일관성 요구 수준에 맞게 해결책을 선택하는 것이 중요하다.
