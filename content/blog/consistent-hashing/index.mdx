...
기본 원리는 Consistent Hashing 알고리즘이 Object와 캐시 시스템 둘 다 동일한 Hash 함수를 사용해서 해싱하는 데 있다. 캐시 시스템은 구간을 정하고 그 구간에는 많은 Object의 해시값을 가지고 있다. 캐시 시스템이 제거되면 인접한 구간의 캐시 시스템이 제거된 구간을 맡게 되고 다른 캐시 시스템은 영향을 받지 않는다.
