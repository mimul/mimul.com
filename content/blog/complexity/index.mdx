...
description: State/Control이 만드는 복잡성의 원인, Essential과 Accidental Complexity의 구분, 분류값이 만드는 조건 분기 리팩터링까지 복잡성 이론과 실천을 정리합니다.
author: Mimul
...
위 네 가지 방법은 상태, 제어, 코드량이라는 "Out of the Tar Pit"의 원인들을 겨냥한다. 그런데 실무에서는 이 목록에 없는 원인이 하나 더 자주 부딪힌다. 분류(구분) 값이다. 회원 등급, 결제 수단, 배송 상태처럼 몇 가지 값 중 하나를 갖는 필드는 그 자체로는 무해해 보이지만, 그 값을 검사하는 조건문이 코드베이스 곳곳에 반복해서 나타나면서 복잡성을 만든다.
