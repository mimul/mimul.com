...
lastModificationTime: "2026-07-21T00:00:00.000Z"
...
여기서 흔히 저지르는 반대 방향의 실수가 classitis다. Ousterhout는 얕은 클래스를 잘게 쪼개기만 하는 습관을 이렇게 부르며 경계한다. 클래스 수를 늘린다고 저절로 깊어지지 않는다. 감추는 지식이 서로 다를 때만 쪼갤 값어치가 있다. 뒤에 나올 지갑 예제에서 이 두 원칙을 코드로 확인한다.
...
이 접근이 실제로 지키려는 값은 유연성이다. 그런데 유연성은 말처럼 단순한 개념이 아니다. kawasima는 [유연성과 범용성](https://scrapbox.io/kawasima/%E6%9F%94%E8%BB%9F%E6%80%A7%E3%81%A8%E6%B1%8E%E7%94%A8%E6%80%A7)에서 유연성(flexibility)과 범용성(generality)을 구분해야 한다고 지적한다. 유연성은 기존 동작을 깨지 않고 합리적인 비용으로 변경을 흡수할 수 있는 정도이고, 범용성은 하나의 구현이 얼마나 넓은 입력과 맥락을 다룰 수 있는지를 가리킨다. 얼핏 같은 말처럼 들리지만 두 축은 독립적으로 움직인다.
...
이렇게 쌓인 격차는 결국 작은 업데이트 여러 번이 아니라 한 번의 큰 마이그레이션으로 돌아온다. 그리고 이 마이그레이션은 정확히 우발적 복잡성의 정의에 들어맞는다. 사용자는 라이브러리가 몇 버전인지 관심이 없다. 그런데도 개발팀은 몇 주씩 이 작업에 매달리고, 그동안 실제 비즈니스 요구사항에 쓸 시간은 줄어든다.
...
- [유연성과 범용성](https://scrapbox.io/kawasima/%E6%9F%94%E8%BB%9F%E6%80%A7%E3%81%A8%E6%B1%8E%E7%94%A8%E6%80%A7)
