...
description: uptime, top, sar, vmstat, iostat 등 핵심 명령어와 root 로그인 실패 감지, 네트워크·디스크·메모리 상태 확인법 등 리눅스 서버 운영에 필요한 모니터링 작업을 정리합니다.
author: Mimul
date: "2019-09-26T22:17:18.000Z"
lastModificationTime: "2026-04-05T00:00:00.000Z"
image: "./LinuxPerformanceObservabilityTool.png"
tags:
  - linux
  - monitoring
---
...
아래는 IP별로 로그인 실패 시도 건수이고 건수가 많은 IP는 블럭킹해준다.
...
`-c`를 쓰면 프로세스 목록 창에 표시되는 프로세스 이름이 인자의 정보도 포함된다. top 화면으로 이동한 다음 1를 입력하면 각 CPU 코어의 활용도를 개별적으로 볼 수 있다.
...
- %iowait가 표시되는 경우 CPU가 I/O 작업을 기다리고 있었음을 나타내는데, 시간의 비율로 보여준다.
- %idle은 CPU가 아무 작업도 하지 않는 유휴 상태의 시간 비율.
...
첫 번째의 출력값은 디스크 장치가 활성화되고 나서 현재까지의 누적 값이며, 현재의 상황을 아는 경우는 두 번째 이후의 출력에서 보여준다. 보통 IOPS[r/s(초당 읽기 섹터 수), w/s(초당 쓰기 섹터 수)]와 %util을 주의깊게 본다.
...
- bash_history 점검 : curl, mysql 등의 커맨드 실행할때 계정 정보를 넣어서 실행하는 것을 감시한다. 아이디와 패스워드가 bash_history에 남아 있어 보안 문제가 대두된다. history에서 제거하려면
  * 아래 명령어를 통해 행번호를 확인한다.
  ```bash
  # history | less
  ```
  * 아래 명령어를 통해 해당 행을 삭제한다.
  ```bash
  # history -d 108
  ```
- 근원적으로 패스워드를 안남게 하려면 아래와 같이 몇가지 경우에 대해 처리 방법을 기술한다.
  * curl의 경우는
  ...
  ```
  * 통해 패스워드 이력을 남기지 않는다. mysql의 경우는 -p 이후에 패스워드를 입력하지 않고 엔터 다음에 패스워드를 입력하면 된다.

### 마치며

서버 모니터링은 단순히 명령어를 외우는 것이 아니라, 각 지표가 의미하는 바를 이해하고 이상 징후를 빠르게 포착하는 데 의미가 있다. uptime으로 전반적인 부하를 확인하고, top/sar/vmstat으로 CPU·메모리 병목을 진단하며, iostat으로 디스크 I/O를 점검하는 흐름을 습관화하면 장애 대응 속도가 크게 달라진다. 보안 측면에서는 로그인 실패 로그와 bash_history 점검을 주기적으로 병행하는 것이 중요하다.
