---
title: Field Injection 대신 Constructor Injection을 써야 하는 이유
description: Spring에서 의존성 주입(DI)이 필요한 이유를 Google Guice 예제로 설명하고, Field/Setter Injection 대신 Constructor Injection을 권장하는 5가지 이유를 정리합니다.
author: Mimul
...
tags:
  ...
  - java
---
재사용성, 테스트 용이성, 코드 단순화, 낮은 결합도 등의 장점 덕분에 DI는 다양한 언어와 프레임워크에서 널리 쓰인다. 이 글에서는 DI가 왜 필요한지를 Google Guice 예제로 살펴보고, Spring에서 제공하는 세 가지 주입 방식 중 Constructor Injection이 권장되는 이유를 정리해본다.

### 왜 DI(의존성 주입)가 필요한가?

DI가 필요한 이유에 대한 좋은 해답으로 [Google Guice Motivation](https://github.com/google/guice/wiki/Motivation) 페이지에서 잘 설명해 주어서 인용해 본다.

**동기요인**

관련된 모든 객체들을 밀결합하는 것은 애플리케이션 개발에서 까다로운 문제가 된다. 애플리케이션에서 서비스, 데이터, 프레젠테이션 클래스들을 연결하는 방법에는 여러 가지가 있다. 이러한 접근 방법을 비교하기 위해 피자 주문에 관련된 빌링 코드를 작성할 것이다.
...
구현과 함께 단위 테스트를 작성할 것이다. 테스트는 실제 신용 카드로 결제가 되지 않도록 가짜 카드 결제를 구현한 FakeCreditCardProcessor 클래스가 필요하다.
...
이 코드에도 문제가 있다. 글로벌 변수가 mock 구현을 보유하기 때문에 setUp과 tearDown에 세심한 주의가 필요하다. tearDown 메소드가 실패하면 글로벌 변수는 테스트 인스턴스를 계속 참조하게 된다. 이렇게 되면 다른 테스트에 문제를 야기할 수 있고, 여러 테스트를 병행할 수 없게 된다.

그러나 가장 큰 문제는 의존성이 코드 속에 숨겨져 있다는 점이다. 만약 CreditCardFraudTracker 의존성을 추가했다고 하자. 테스트가 실패했을 때, 어떤 클래스에 문제가 있었는지 파악하려면 테스트를 다시 실행해야 한다. 운영 서비스에서 Factory 초기화를 잊으면 과금이 발생할 때까지 문제를 알 수 없다. 애플리케이션이 비대해질수록 세심한 주의가 필요한 Factory 클래스는 생산성을 떨어지게 하는 원인이 된다.

품질 문제는 QA나 인수 테스트에서 발견할 수 있다. 지금으로도 충분할지도 모르지만, 확실히 더 나은 방법이 있다.

**의존성 주입(DI)**

Factory처럼 DI도 디자인 패턴이다. 핵심 원칙은 행태와 의존성 해결을 분리하는 데 있다. 여기 예제에서 보면, RealBillingService는 TransactionLog와 CreditCardProcessor를 lookup하는 책임을 지지 않는다. 대신 그들은 생성자를 인수로 전달한다.
...
다음으로, Spring에서 제공하는 세 가지 DI 방식을 살펴보고 Spring 팀이 Constructor Injection을 권장하는 이유를 정리한다.
...
생성자의 인자가 많을 경우 코드량도 많아지고, 의존관계도 복잡해져 단일 책임의 원칙에 위배된다. Constructor Injection을 사용하면 의존관계와 복잡성을 한눈에 알 수 있어 리팩토링의 단초를 제공하게 된다.
...
의존 객체 중 필수는 Constructor Injection을 옵션인 경우는 Setter Injection을 사용할 수 있다.

### Lombok을 사용한 Constructor Injection
