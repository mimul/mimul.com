---
title: OpenLDAP 설치·설정부터 Jenkins, Gitlab 연동까지
description: OpenLDAP 서버를 구축하고 메일 스키마 설계, 사용자 데이터 등록, Gitlab·Jenkins LDAP 연동 설정까지 단계별로 정리합니다.
author: Mimul
date: "2010-02-05T01:00:17.000Z"
lastModificationTime: "2026-04-08T00:00:00.000Z"
image: "./jenkins-gitlab-nexus-ldap.png"
tags:
  - ldap
  - jenkins
  - gitlab
  - nexus
---
Google이 3rd Party용 무료 이메일을 지원하지 않는 이유로 자체 LDAP(OpenLDAP) 기반의 메일 서버를 운영하는 스타트업이 있을 것이다. 내부 시스템 정비 차원에서 LDAP을 도입하는 경우도 많다. 그리고 B2B 소프트웨어를 만들어서 기업에 포팅하다 보면 기존 기업의 사내 LDAP 연동을 통해 인증과 권한 관리 기능을 자체 제품에 녹아내려면 LDAP 기반의 메일 및 사내 직원 인증 서버를 구축해서 테스트하기에도 용이하다.

LDAP은 메일(Postfix + Dovecot) 발송/수신/인증 서버, Gitlab(소스 저장소), Jenkins(빌드), Nexus 등의 시스템과 인증 부분을 연동해서 사용한다.
이후부터는 LDAP을 사용하기 위한 설치 및 설정 방법을 기술한다.
...
**2. slapd.conf의 rootpw에는 암호화된 문자열 생성 방법**
...
OpenLDAP의 slapd.conf에서 mail.schema를 include한다.
...
LDAP 툴은 Apache Directory Studio가 가장 좋아 보이니 다운받아서 사용하면 된다. 맥용, 윈도우용 모두 있다.
...
`Configure Global Security > Access Control > Security Realm > LDAP` 을 선택하고 설정 정보는 아래와 같다.
...
OpenLDAP 기반의 인증 서버를 구축하면 Gitlab, Jenkins 등 다양한 내부 시스템의 계정을 단일 디렉토리로 일원화할 수 있다. 계정 추가·삭제·비밀번호 변경을 LDAP에서 한 번만 처리하면 연동된 모든 시스템에 즉시 반영되므로, 계정 관리 부담을 크게 줄일 수 있다.
