---
title: NULL 반환이 나쁜 이유와 OOP에서의 올바른 대안
description: 객체지향 프로그래밍(OOP) 환경에서 NULL 값의 사용은 설계적 결함이며, 가능한 한 피해야 하고, 코드의 가독성, 안정성, 유지보수성 모두를 해친다고 보고, 대안으로 Null Object 패턴 혹은 예외(Exception) 던지기 방식을 제시합니다.
author: Mimul
...
tags:
  - programming
---
개발하면서 항상 마주치는 것이 NULL 체크다. NULL을 반환할지 말지의 판단을 개발자에게 전가하는 것은 좋지 않은 설계다. [Why NULL is Bad?](https://www.yegor256.com/2014/05/13/why-null-is-bad.html)를 번역하며 그 이유를 정리한다.
...
이 메서드 무엇이 잘못되었는가? 객체 대신 NULL을 반환할 수 있다는 것이 잘못된 것이다. NULL은 객체 지향 패러다임의 끔찍한 관습이며, 온 힘을 다해 피해야할 것 중에 하나이다. 이에 대해서는 많은 의견이 이미 발표되어 있다.
...
### 임기응변적 오류 처리 (Ad-hoc Error Handling)
...
OOP는 주로 이런 임시 오류 처리의 블록을 없앨 목적으로 예외 처리를 도입했다. OOP에서는 예외 처리를 애플리케이션 레벨에서
...
해결책? 지연로드는 위의 예 같은 원시적인 방법으로는 사용하지 마라. 대신 캐시 문제를 애플리케이션의 다른 레이어에 옮겨라.
...
이 분석이 설득력이 있었다면, 이제부터라도 NULL 사용을 멀리하길 권한다.
