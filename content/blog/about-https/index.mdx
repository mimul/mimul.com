---
title: HTTPS 적용과 보안 설정을 위해 알아야 할 TLS 핸드셰이크와 취약점 정리
description: TLS 핸드셰이크 동작 원리부터 BEAST, POODLE 등 SSL 주요 취약점, Cipher Suites 설정 방법, 브라우저별 TLS 1.1 지원 중단 일정까지 HTTPS 적용에 필요한 핵심 내용을 설명합니다.
author: Mimul
...
tags:
  ...
  - tls
---
인터넷 사이트의 안전을 위해 HTTPS를 적용하는 사이트가 늘고 관심도 커지고 있다. SSL 인증서를 구매해 서버에 설치한다고 끝나지 않는다. HTTPS 프로토콜과 동작 방식을 제대로 이해해야 사이트의 신뢰성을 높이고 문제 발생을 줄일 수 있다. 이런 내용을 리서치하고 조사해 공유한다.
...
[https://mimul.com](https://mimul.com)을 접속하면 브라우저는 mimul.com에서 인증서를 수신하기 때문에 접속한 mimul.com이 진짜 mimul.com임을 증명해 준다.
...
새로운 프로토콜 HTTP/2, QUIC에서, 그리고 최신 API 경우 getUserMedia(video, audio), WebRTC, ServiceWorker, Geolocation, Push notification, Progressive Web App 등에서 실행 허가가 필요한데 HTTPS가 실행 허가에 중요한 요소이다.

**5. 기타**

Google은 HTTPS 페이지를 [우선적으로 색인을 생성](https://webmasters.googleblog.com/2015/12/indexing-https-pages-by-default.html), SEO 혜택 등.

## HTTP와 HTTPS의 차이

HTTP의 경우는 TCP 핸드셰이크 후에 HTTP 요청/응답으로 서로 애플리케이션 데이터를 주고 받는다. 이때 통신은 평문으로 이루어지고, 중간자 HTTP 데이터를 보거나 변조가 가능하다. 반면, HTTPS는 HTTP over TLS를 말하며, TCP 핸드세이크 후에 TLS 핸드셰이크를 진행하고 그후부터는 암호화 통신이 시작되며 HTTP 요청/응답을 통해 애플리케이션 데이터를 주고 받는다.

![HTTP vs HTTPS](./ssl_http_https_1.png)

## TLS 핸드셰이크 프로토콜

HTTPS는 HTTP over TLS를 말한다. 그래서 TLS의 핸드셰이크 프로토콜을 이해하는게 필요하다.

![TLS 핸드 셰이크 프로토콜](./ssl_tls_1.png)
...
![TLS 핸드 셰이크 resumption](./ssl_tls_resumption_1.png)
...
| [TLS1.1](https://tools.ietf.org/html/rfc4346) | ◯ | TLS 1.0에서 발견된 새로운 공격기법(BEAST 공격 및 CBC 모드의 오류를 사용한 공격)에 대한 대응. 2006년에 공개. [PCIDSSv3.2](https://blog.pcisecuritystandards.org/are-you-ready-for-30-june-2018-sayin-goodbye-to-ssl-early-tls) |
...
| [TLS1.3](https://tlswg.github.io/tls13-spec/) | 개발중 | 인터넷 환경의 변화와 TLS1.2까지의 암호화 강도 부족을 개선하기 위해 만들기 시작했고 2018년 8월에 [RFC8446](https://datatracker.ietf.org/doc/rfc8446/)으로 스펙이 결정됨.|
...
> 데이터를 암호화하기 전에 압축되었을 때 발생하는 정보의 유출이다. 만약 민감한 정보와 비교적 예측 가능한 데이터 콘텐츠에 주입되어 섞여 있었을 때, 암호화된 스트림을 관찰할 수 있다면 공격자는 그 때 알 수 없는 데이터를 추출할 수 있다.
...
> 이것은 CRIME에 비해 더 복잡한 공격이며, TLS 수준의 압축을 필요로하지 않는다(HTTP-level의 압축은 필요함). 이 공격이 성공하려면 다음의 요건이 필요하다:
  > HTTP 수준의 압축을 사용함.
...
> RC4 알고리즘은 초기화 단계 상태의 데이터와 키 데이터를 적절하게 결합하지 않기 때문에 스트림의 첫 번째 바이트에 일반 텍스트 복구 공격(plaintext-recovery attack)을 실행하는 취약점이 존재한다.
...
HTTPS 적용은 인증서 설치로 끝나는 게 아니다. TLS 버전, Cipher Suites, 취약한 알고리즘 비활성화까지 설정을 제대로 갖춰야 실질적인 보안이 확보된다. [Qualys SSL Labs](https://www.ssllabs.com/ssltest/analyze.html)로 현재 사이트의 등급을 확인하고, [Mozilla SSL Configuration Generator](https://ssl-config.mozilla.org/)로 웹 서버별 권장 설정을 적용하는 것을 출발점으로 삼을 것을 권장한다.

## 참조 사이트

- [Measuring HTTPS Adoption on the Web](https://static.googleusercontent.com/media/research.google.com/en//pubs/archive/46197.pdf)
- [SSL and TLS Deployment Best Practices](https://github.com/ssllabs/research/wiki/SSL-and-TLS-Deployment-Best-Practices)
- [RateLimit Header Fields for HTTP Draft](https://tools.ietf.org/html/draft-polli-ratelimit-headers-01)
- [RFC5246 - TLS 1.2](https://tools.ietf.org/html/rfc5246)
- [RFC8446 - TLS 1.3](https://datatracker.ietf.org/doc/rfc8446/)
