...
lastModificationTime: "2018-11-04T20:51:42.000Z"
...
MySQL의 EXPLAIN 출력은 테이블 형태지만 내부적으로는 트리로 표현되므로, VISUAL EXPLAIN 툴을 사용하지 않는 한 트리 형태로 이해할 필요가 있다. 그래야만 각 테이블이 어떻게 액세스되는지 쉽게 이해할 수 있다. 이 글에서 실행 계획을 id/select_type/table, type/key/ref/rows, Extra 세 그룹으로 나눠 정리를 했다.

**id와 select_type** 은 EXPLAIN의 처음 두 필드이지만 이들은 하나의 세트로 생각하면 좋다.

select_type은 질의의 종류를 나타내며, 주제 트리의 구조에 그대로 반영된다. 쿼리의 종류는 JOIN, 서브 쿼리, UNION 및 이들의 조합으로, 이 필드의 내용도 그 조합으로부터 도출된 것이다.

JOIN의 경우 select_type은 SIMPLE이고 id는 동일하다. EXPLAIN의 출력 순서가 어떤 테이블에서 처리할 것인가에 대한 순서를 반영하고 있다. MySQL은 기본적으로 Nested Loop Join으로 테이블을 순회하며 인덱스가 없는 테이블은 Block Nested Loop로 조인하지만, 8.0.18부터는 적합한 인덱스가 없는 동등 조인(equi-join)에 한해 Hash Join도 선택할 수 있다. 이 경우 Extra에 `Using join buffer (hash join)`으로 표시되며, id와 select_type의 규칙 자체는 그대로 적용된다.
...
테이블에 액세스하는 방법을 나타낸다. 치명적인 쿼리는 이 필드를 보면 한눈에 알 수 있어 가장 중요한 필드다. 성능은 `system > const > eq_ref > ref > ref_or_null > index_merge > range > index > ALL` 순으로 좋다.

| 필드 값       | 설명                                                                                            |
| :---------- | :--------------------------------------------------------------------------------------------- |
| system      | 테이블에 행이 하나뿐인 특수한 경우의 액세스 타입. const의 한 형태로, 앞서 살펴본 예제에서 derived3 테이블의 type이 system인 것도 이 때문이다. |
...
| index_merge | 여러 인덱스를 각각 조회한 뒤 그 결과를 병합하는 방식. WHERE 절에서 서로 다른 인덱스 컬럼이 OR로 연결된 경우 등에 선택된다. |
...
또한 가져온 모든 행이 그대로 결과로 반환되는 것은 아니다. Using where가 Extra 필드에 표시되어 있는 경우 패치한 행에 대해서 WHERE구의 검색 조건이 더 적용되어 행의 세분화가 행해지므로 클라이언트에 돌려주어지는 결과 행은 적어질 가능성 있다. JOIN을 처리하는 경우 WHERE 절에 의해 행의 좁히기가 없으면 최종 결과 행수는 JOIN 하는 모든 테이블의 rows 필드의 곱으로 생각할 수 있다. 테이블이 서너 개만 조인되어도 각 테이블의 rows가 곱해지면서 값이 기하급수적으로 커지므로, rows가 큰 테이블이 조인 순서 앞쪽에 있지는 않은지 살펴볼 필요가 있다.

스토리지 엔진이 초당 처리할 수 있는 행 수에는 물리적인 상한이 있다. 그래서 쿼리 튜닝은 결국 각 테이블에서 패치되는 rows를 얼마나 줄일 수 있는가로 귀결되며, 이는 실행 계획을 읽을 때 가장 먼저 확인해야 할 기준이 된다.
...
| Not exists                                  | LEFT JOIN에 오른쪽 테이블 컬럼이 NULL인 행만 찾는 조건(`WHERE t2.col IS NULL`)이 있고 그 컬럼이 NOT NULL로 정의되어 있을 때 표시됨. 이 조건을 만족하는 행은 오른쪽 테이블에 매치되는 행이 없는 경우뿐이므로, 옵티마이저는 매치 여부만 확인하고 나머지 행 탐색을 생략하는 최적화를 적용함 |
...
세 그룹 중 어디를 보든 판단 기준은 하나로 모인다. 각 테이블에서 패치되는 rows를 줄일 수 있는가이다. 쿼리를 조금씩 고쳐 쓰면서 EXPLAIN 출력이 어떻게 바뀌는지 비교하는 반복이, 실행 계획을 읽는 감각을 가장 빠르게 익히는 방법이다.
