...
- 명시성은 가독성과 AI 추론 품질의 핵심이다. Explicit Flow(흐름이 명시적이고 직관적으로 드러나는 코드), Visible State Transition(상태 전환이 명확하게 보이고 추적 가능한 설계), Predictable Behavior(코드를 읽었을 때 예상대로 동작이 예측 가능한 코드)를 생각한다.
...
- 관련된 로직은 가까이 배치하여 cognitive jump를 최소화한다. Cohesive Module(관련된 책임이 잘 모여 응집도가 높은 모듈), Shallow Structure(계층이 얕고 구조가 평평하며 복잡도가 낮은 설계), Explicit Dependency(의존성이 명시적이고 숨겨지지 않은 설계) 추구하자.
...
- 모든 추상화는 유지보수 비용을 가진다. 추상화는 오직 인지 부하를 줄이고 반복되는 개념을 안정화하며 변경 비용을 낮출 때ㅁ난 한다.
- 복잡한 orchestration보다 단순하고 명시적인 흐름을 선호한다. explicit branching(분기 처리가 명시적이고 이해하기 쉬운 구조), predictable execution(실행 흐름이 예측 가능하고 안정적인 코드), obvious data flow(데이터의 흐름이 명확하고 쉽게 추적 가능한 설계)
- 잘못된 추상화는 복잡성을 시스템 전체로 전파한다. 작은 지역 중복(local duplication)은 종종 premature abstraction, unstable shared logic, forced reuse 보다 안전하다.
