...
description: Apigee의 Web API Design 가이드를 바탕으로 RESTful API 설계 원칙 18가지를 정리합니다. URL 설계, 버전 관리, 오류 처리, 페이징 등 실무에서 바로 적용할 수 있는 Best Practice를 다룹니다.
author: Mimul
...
tags:
  ...
  - apigee
---
기업과 개발자들에게 API 제품 및 기술을 공급하는 Apigee는 Web API 설계에 관한 무료 책 [Web API Design : Crafting Interfaces that Developers Love](https://pages.apigee.com/rs/apigee/images/api-design-ebook-2012-03.pdf)를 발표했다.

여기에는 좋은 API를 설계하기 위한 Best practices가 정리되어 있어 API를 도입하는 분들에게 좋은 노하우를 제공하고 있다. 본 포스트에서는 이 책을 참고하여 좋은 API 설계란 무엇인지 정리한다.
...
- HTTP 상태코드를 정하고(많아도 안 좋음), 개발자들을 위한 오류 메시지 정의, 상세 정보 링크 등을 넣어주면 좋다.
```text
200 - OK
400 - Bad Request
...
- 기본 형식은 json이다.
- Accept 헤더에 타입을 지정하거나, URL 속에 type 매개변수를 사용할 수 있다. 권장 방식은 명사.type(도트 형식의 서식)으로 하는게 낫다.
```text
...
- 여러 기능적으로 독립된 url을 여러 개 만들지 말고 모든 API 요청을 하나의 API 하위 도메인에 정리하자. api.company.com 같은 것을 사용하는 것이다.
