---
title: LangChain 기반 LLM 애플리케이션 개발 기초 — 프롬프트, 모델 정의, 실행 (1)
description: LangChain으로 LLM 애플리케이션을 개발할 때 필수적인 프롬프트 작성법, 모델 정의 방법(Gemini·OpenAI·Anthropic 등), invoke/stream 실행 패턴을 코드와 함께 단계별로 설명합니다.
author: Mimul
date: "2024-05-21T20:44:25.000Z"
lastModificationTime: "2026-04-04T00:00:00.000Z"
image: "./langraph-langchain-llm.png"
tags:
  - langchain
  - llm
  - ai
---
LangChain과 LangGraph를 이용하여 Deep Research와 같은 LLM 애플리케이션을 만드는 방법에 대한 첫 번째 글로 "LangChain 기반 LLM 애플리케이션 개발 기초"를 다룹니다. 이 글을 완독하신 분은 아래 링크에서 두 번째, 세 번째 글을 읽기를 권장합니다.

- [LCEL(LangChain Expression Language) 기반 LLM 애플리케이션 개발 기초 (2)](https://www.mimul.com/blog/langchain-lcel/)
...
이 글에서는 LangChain을 활용한 LLM 애플리케이션 개발에 기본적으로 필요한 세 가지를 다룹니다. 프롬프트를 만드는 방법, 모델을 정의하는 방법, 그리고 프롬프트와 모델을 실행하는 방법입니다.
...
- ```model="gemini-2.0-flash"```은 사용할 Google AI 모델의 이름을 지정한다.
...
여기에서는 프롬프트 템플릿과 정의한 모델을 결합하여 LLM을 호출하고 응답을 얻는 절차에 대해 기술한다. run, apply, generate, predict 등 다양한 방법의 메소드들이 존재하나, 최근에 권장 사항은 invoke 메소드에 의한 실행을 추천한다.
...
- ```model.invoke(llm_prompt1)```에서 model은 ChatGoogleGenerativeAI를 사용하여 정의한 모델 객체이며 .invoke()메서드는 파라미터로 입력된 프롬프트(```llm_prompt1```)를 LLM에 보내고 받은 응답을 response1 변수에 저장한다.
...
```
################### model output ###################
content='알겠습니다. 한국에서 가장 높은 산에 대해 자세히 알려드리겠습니다...' additional_kwargs={} response_metadata={'finish_reason': 'STOP', ...} usage_metadata={'input_tokens': 59, 'output_tokens': 1099, 'total_tokens': 1158, ...}
content='알겠습니다. 한국에서 가장 높은 산에 대해 자세히 알려드리겠습니다...' additional_kwargs={} response_metadata={'finish_reason': 'STOP', ...} usage_metadata={'input_tokens': 59, 'output_tokens': 1092, 'total_tokens': 1151, ...}
################ model output text #################
...
LangChain에서 Stream 출력을 만들기 위해 Callback 함수를 구현하는 경우도 있지만, stream과 astream을 이용하는 방식이 권장됩니다.

프롬프트 템플릿으로 지시 형태를 정의하고, invoke로 구체적인 입력을 생성한 뒤, 모델 invoke로 LLM을 실행해 응답을 얻는 흐름이 LangChain 기반 LLM 애플리케이션의 기본 패턴입니다. 다음 글에서는 이 패턴을 LCEL(LangChain Expression Language)로 체이닝하는 방법을 다룹니다.
