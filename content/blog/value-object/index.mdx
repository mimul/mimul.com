...
lastModificationTime: "2018-05-14T10:00:00.000Z"
...
public 생성자로도 검증은 가능하다. Smart Constructor의 차이는 "검증을 거치지 않고는 이 객체를 만들 수 없다"는 것을 구조적으로 강제한다는 점이다. 호출자가 실수로 `new Email("invalid")`처럼 검증 경로를 우회할 여지 자체를 없앤다. `Email` 타입을 받는 메서드는 검증을 다시 할 필요가 없다. 타입 자체가 "이미 검증된 이메일"임을 보장하기 때문이다. 이렇게 프리미티브 타입을 감싸 만든 타입을 함수형 진영에서는 브랜드 타입(branded type)이라 부르기도 한다. 이름은 다르지만 가리키는 것은 같다. 검증을 통과한 원시값에 타입이라는 표식을 붙여, 표식이 없는 값과 컴파일러가 구분할 수 있게 만드는 것이다.
...
값 객체는 속성값뿐 아니라 다른 객체를 가리키는 식별자를 감싸는 데도 쓰인다. 주문이 고객을 참조할 때 `Customer` 객체를 통째로 들고 있는 대신, `CustomerId`처럼 식별자 하나만 감싼 값 객체로 참조하는 식이다. `Long customerId`와 `Long orderId`를 그대로 쓰면 두 값을 바꿔 넘겨도 컴파일러가 잡지 못하지만, 각각을 별도의 값 객체로 감싸면 이런 실수는 컴파일 타임에 드러난다. 객체 참조 대신 식별자로만 연결되므로 두 대상 사이의 결합도 그만큼 느슨해진다.
