...
tags:
  ...
  - ai-tools
---
이 글은 Claude Code를 실무에 도입하면서 정리한 설정 방법과 코드 리뷰 자동화 경험을 담은 글이다. Claude Code의 설정 구조를 파악하고 나서 코드 리뷰 프로세스를 이해하면 전체 흐름을 훨씬 빠르게 파악할 수 있어 그 순서로 설명한다. Claude Code를 처음 설정하거나 AI 기반 코드 리뷰 자동화에 관심 있는 개발자를 대상으로 한다. 참고로 예시에 사용된 프로젝트는 Rust 프로젝트다. 설정 부분에 Rust 코딩 가이드가 일부 포함되어 있으나, Rust를 몰라도 전체 흐름을 이해하는 데 문제없다. 마지막으로 코드 리뷰의 지적(실패) 사항들을 학습하는 루프를 만들어서 코드 리팩토링을 개선하는 방법도 기술한다.
...
이번에는 코드 리뷰 프로세스에 따라 Claude를 활용한 예시를 설명한다. 먼저 아래에 코드 리뷰 프로세스를 보고 전체 흐름을 이해하기 바란다.
...
더 자세한 내용은 [SKILL.md](https://github.com/mimul/axum-rusty/blob/main/.claude/skills/code-review-feedback-rust/SKILL.md)를 보기 바란다. 
...
그리고 지적사항에 대해 코드를 수정 진행을 하고 난 다음 사람의 승인을 받고 Github에 푸시를 한다. 그리고 코드 수정한 내용은 아래와 같이 요약해서 출력해 준다.
