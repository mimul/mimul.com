...
먼저 rules의 [coding-style.md](https://github.com/mimul/axum-rusty/blob/main/.claude/rules/coding-style.md)를 기반으로 언어 공통적으로 적용할 룰과 보안 security.md와 테스트 test.md를 담당하는 룰이 있어 이를 기반으로 코딩 리뷰 프로세스가 움직인다. 그리고 프로그래밍 언어별로 특화사항을 검토해 체크리스트와 패턴들을 상세화해서 skills에 정의하면 코드리뷰 기준들이 완성된다.
