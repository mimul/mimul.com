...
우선 순위는 **엔터프라이즈 → 사용자 레벨 → 공유 프로젝트 → 로컬 개인** 순이다.
...
설정 구조를 파악했으니, 이를 실제 코드 리뷰 자동화에 어떻게 적용하는지 살펴본다. 아래 예시는 Skills와 Commands를 조합해 리뷰 → 대응 → 회신 전 과정을 Claude가 보조하도록 구성한 것이다. 먼저 코드 리뷰 프로세스를 보고 전체 흐름을 이해하기 바란다.
