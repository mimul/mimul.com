---
title: Git 내부 구조와 커밋·인덱스 동작 원리 이해
description: blob, tree, commit, tag 4가지 Git 객체의 내부 구조와 git add·commit 시 내부에서 일어나는 동작 원리를 실제 파일과 명령어로 단계별로 설명합니다.
author: Mimul
...
tree 객체는 디렉토리의 정보를 보관 유지하고 있다. 여기는 desc 디렉토리의 어느 시점의 tree 객체이다. 이 tree 객체가 만들어지면 그 디렉토리에 존재하는 파일과 그 버전의 blob 해시값(Key)이 기록된다. 이 Key에서 해당 blob 객체에 액세스할 수 있다. 루트 디렉토리에 해당하는 tree 객체는 아래와 같다.
...
.git/HEAD에 저장되어 있으며, commit이나 checkout으로 변경된다. 브랜치 이름으로 checkout한 경우 커밋 해시가 아닌 브랜치 경로(예: `ref: refs/heads/master`)가 기록된다.
...
commit 객체와 비슷한 구조로 되어 있으며 메시지 등의 정보를 기록하고 싶은 경우에는 annotated tag의 사용을 검토해보자.

다시 한번 commit의 프로세스를 정리하면 다음과 같다.

1. 코드를 편집하고 `git add`를 실행하면 blob 객체가 생성되고 인덱스가 업데이트된다.
2. `git commit`을 실행하면 전체 리포지토리에 대한 tree 객체가 생성된다.
3. commit 객체가 생성되고 HEAD가 새 커밋 해시로 갱신된다.
