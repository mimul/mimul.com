---
title: JavaScript Promise 동작 원리를 코드로 이해하기
description: JavaScript Promise의 내부 동작을 직접 구현하며 이해합니다. pending/resolved/rejected 상태 전환, then() 메소드 체이닝, 오류 처리까지 단계별로 구현 원리를 설명합니다.
author: Mimul
...
tags:
  ...
  - promise
---
JavaScript에서 비동기 처리가 많아지면 콜백이 중첩되어 가독성과 디버깅이 어려워진다. 이를 해결하는 방법 중 하나가 Promise다. 대부분 라이브러리 형태로 제공되어 내부 동작을 모르고 사용하다 오용하는 경우가 생긴다. 이 글은 Promise 내부를 직접 구현하며 동작 원리를 파악하는 데 초점을 맞춘다.

소개 원문은 [JavaScript Promises ... In Wicked Detail](http://www.mattgreer.org/articles/promises-in-wicked-detail/) 이다.
...
이 구현은 Callback 패턴의 단순한 대체이며, 이것만으로는 큰 의미를 갖지 않는다. 그러나 아주 간단한 구현이었지만, Promise의 핵심적인 아이디어를 이미 이해했다고 볼 수 있다.

Promise는 객체안에 궁극적으로 처리 결과값을 수집한다.
...
Promise를 사용하면 then()과 resolve()의 호출 순서를 고려하지 않아도 된다. then()과 resolve()는 우리의 이용 목적에 따라 언제라도 호출할 수 있다. 이것은 Promise 처리 결과값을 객체에 저장하는 데 아주 큰 장점 중에 하나다.
...
Promise를 더 잘 활용하려면 라이브러리의 all() 메소드나 다양한 유틸리티 메소드를 사용하면 유용성이 더욱 높아진다. 무엇을 사용 하는가는 독자 여러분의 취향에 맞게 선택하면 된다.
