---
title: MySQL innodb_flush_method 설정별 성능 비교와 튜닝 가이드
description: MySQL InnoDB의 innodb_flush_method 매개변수인 fdatasync, O_DIRECT 각 설정의 의미와 Direct I/O 원리를 설명하고, 버퍼 풀 크기별 성능 테스트 결과를 바탕으로 튜닝 기준을 제시합니다.
author: Mimul
...
MySQL InnoDB innodb_flush_method 매개 변수는 Unix/Linux에서 데이터 파일, 로그 파일을 읽고 쓰는 방법을 지정하는 것으로 [InnoDB Startup Options and System Variables](https://dev.mysql.com/doc/refman/8.0/en/innodb-parameters.html#sysvar_innodb_flush_method)에 따르면 다음과 같은 세 가지 설정이 가능하다.
...
O_DIRECT 플래그를 사용하여 파일을 열면 OS가 가지고 있는 Direct I/O 기능을 이용할 수 있다. 즉, 이것은 OS의 캐시 메커니즘을 무시하는 기능이며, RDBMS는 자체적으로 데이터를 캐시하므로 OS의 캐시 메커니즘이 필요 없다는 의미이다.

![Direct I/O](./mysql_o_direct.png)
...
RDBMS는 내부에 파일 캐시와 같은 도구를 갖추고 있다. 따라서 파일 캐시를 통하게 되면 캐시 처리가 2중으로 되어 불필요한 지연이 발생하여 Direct I/O와 같이 파일 캐시를 경유하지 않고 직접 디스크에 액세스하는 방안이 필요한 것이다.
...
![테스트 결과](./fdata_o_direct_test.png)

buffer_pool이 128M 이전은 fdatasync 방식이 성능이 더 높고 128M 이후부터는 O_DIRECT 방식이 더 높은 성능 결과를 보여주고 있다.
...
반대로 버퍼 풀이 충분히 커지면 InnoDB가 대부분의 데이터를 자체 캐시에서 처리할 수 있어 OS 파일 캐시의 이중 버퍼링이 오히려 오버헤드가 되며, 이때 O_DIRECT가 OS 캐시 계층을 제거해 더 좋은 성능을 발휘하게 된다.
...
중 하나의 조건을 충족해야 좋은 성능을 발휘할 것으로 보인다. 그리고 O_DIRECT를 사용하는 장점은 Double buffering(MySQL Buffer Pool과 OS 캐시)을 막아 메모리를 효율적으로 사용하고, OS 캐시의 오동작을 회피할 수 있다.

데이터베이스 이외의 기능에서 File I/O가 많이 일어난다면, 트랜잭션의 양, 트랜잭션의 중량감 등도 고려되어야 하고 무조건 버퍼 풀을 크게 잡는 것도 문제이니, 반드시 충분한 테스트를 통해 시스템 환경에 맞는 매개변수를 선택해야 한다. 이론적 예측보다는, 좋다고 덥석 설정 값을 고정하기보다는 테스트로 증명하는 것이 더 좋은 방법이다.
