---
title: MCP(Model Context Protocol) 개요, 아키텍처, 동작 원리 가이드
...
MCP 클라이언트가 모델 요청을 받아 MCP 서버의 해당 기능을 호출합니다(예: Weather API 실행). 서버가 외부 API 또는 데이터베이스를 호출하고 결과를 클라이언트에 반환합니다.
