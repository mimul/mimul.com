...
description: Part Affinity Fields 기반 Bottom-Up 방식으로 실시간 다인 자세 추정을 수행하는 논문을 리뷰하고, Chainer로 FMS 동작 인식 모델을 직접 구현한 과정을 설명합니다.
author: Mimul
...
tags:
  ...
  - fms
---
피트니스 회원의 FMS(Functional Movement Screen) 7가지 운동 동작을 딥러닝으로 자동 평가하는 프로젝트를 진행하면서, Human Pose Estimation에 대한 이해가 먼저라 판단했다. 관련 논문을 살펴보다 [Realtime Multi-Person 2D Pose Estimation using Part Affinity Fields](https://arxiv.org/abs/1611.08050)가 가장 적합하다고 판단해 논문을 먼저 이해하고, 이후 FMS 동작 영상을 가지고 오픈 소스를 참고해 동작 인식 모델을 구현·테스트했다. 이 글에서는 논문 리뷰와 구현 과정을 정리한다.
...
- 공개 : 2016년 11월 24일
- 논문 위치 : [https://arxiv.org/abs/1611.08050](https://arxiv.org/abs/1611.08050)
...
![Bottom-Up 방식](./pose_botton_up.png)
...
![제안된 방식](./pose_proposal.png)

### 3. 논문에서 제안된 아키텍처

**① 제안된 아키텍처 흐름**

![제안 아키텍처 흐름](./pose_architecture1.png)
...
![Part Confidence Maps & Part Affinity Fields](./pose_architecture2.png)
...
- Branch1에서 confidence map을 예측, Branch2에서 part affinity fields을 예측.
...
![Part Confidence Maps & Part Affinity Fields 2](./pose_architecture3.png)
...
![Bipartite Matching](./pose_architecture4.png)
...
![Loss function 1](./pose_arithmetic11.png)
...
![Part Affinity Fields 정답치 데이터 생성 1](./pose_arithmetic31.png)
...
1. Chainer 버전의 OpenPose 모델을 가져온다.
...
3. 시각화 코드를 제공하고 있어서 이미지에 인식한 결과를 겹쳐 보이게 한다.

```python
img = draw_person_pose(img, poses)
```

추론 결과의 구조를 확인하기 위해 로그를 추가해 본다.
...
결과 로그를 해석하면 shape의 첫 번째 값 1은 감지된 사람 수, 두 번째 값 18은 관절의 숫자로 고정되어 있으며 순서대로 코, 목, ...과 같이 entity.py의 JointType 클래스를 보면 된다. 세 번째 값 3은 X 좌표, Y 좌표, 플래그가 되고 관절이 없으면 플래그에 0 값이 채워진다. 관절의 정의에 대한 원본은 [여기](https://github.com/CMU-Perceptual-Computing-Lab/openpose/blob/master/doc/output.md#pose-output-format)에 보면 정의되어 있다.
...
논문 리뷰와 Chainer 기반 구현을 통해 Part Affinity Fields를 이용한 Bottom-Up 방식이 Top-Down 방식 대비 속도와 정확도 모두 실용적인 수준임을 확인했다. FMS 동작 인식에서도 기존 trained model이 Deep Squat, Hurdle Step 등 주요 동작을 무난하게 인식하는 결과를 얻었다. 앞으로 몇 가지 논문을 더 검토한 후, 코치가 평가한 FMS 점수와의 유사도를 높이는 방향으로 학습 데이터 구성과 모델 개선을 이어갈 예정이다.
...
- [Chainer version of Realtime Multi-Person Pose Estimation](https://github.com/DeNA/Chainer_Realtime_Multi-Person_Pose_Estimation)
