---
title: Bloom Filter 핵심 정리, 작동 원리부터 수학 이론과 Java 구현까지
...
- **Bloom Filter Key**: 해시 알고리즘이 반환한 키
- **Bloom Filter Index**: Bloom Filter Key를 임의 크기의 비트 블록으로 나눈 하나하나의 조각
- 블룸 필터는 추가하는 add()와 존재 여부를 확인하는 isExist(), 이렇게 두 가지 연산을 제공한다.
- **false positive**: 없는데 있다고 판단하는 경우, **false negative**: 있는데 없다고 판단하는 경우

**2. 작동 방식**

예를 들어, m(bloom filter의 bit 사이즈) = 10, k(hashing functions의 수) = 2로 하자. 2개의 해시 함수를 h1, h2 하고 Bloom Filter 배열은 아래와 같다.
...
즉, 각 Value를 저장하는 대신에 Bloom Filter는 Key가 존재하는 지점(해시 함수에 의해)을 가리키는 Bits의 배열이라고 봐도 된다. 찾는 비트의 조합이 모두 1일 경우 제대로 찾을 확률이 높아진다.
...
- 스펠링체크, 사전, 웹 검색, IP Filtering, Router 등에 쓰이고 Squid Web, Venti Storage System, SPIN model checker, Google Chrome Browser 등에도 쓰이고 있다.
- **Cassandra**: SSTable 생성 시 인덱스 용도로 사용해 Read 성능을 향상시킨다(디스크 I/O 감소)
  * [SMHasher & MurmurHash hash 함수 사용](https://github.com/aappleby/smhasher)
- **HBase**: HFile 안에 로우와 컬럼이 존재하는지 검사하기 위해 사용한다
- **Bigtable**: 불필요한 디스크 접근을 피하기 위해 사용한다
- **Oracle**
  * Parallel Join 시 Slave 간의 communication 데이터량을 줄이기 위해(10gR2)
  * Join-Filter Pruning 사용 시(11gR1)
