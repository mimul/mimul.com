---
title: 읽기 쉬운 코드를 위한 변수와 메소드 네이밍 모범 사례 15가지
description: 변수와 메소드 이름을 일관되고 명확하게 짓는 방법을 정리한 15가지 네이밍 원칙입니다. 좋은 네이밍 습관을 익히면 코드 리뷰와 협업 속도가 함께 빨라집니다.
author: Mimul
date: "2012-03-01T17:50:16.000Z"
lastModificationTime: "2012-03-01T22:19:01.000Z"
image: "./style_guide.jpeg"
tags:
  - programming
  - naming
---
코딩 스타일을 좋게 하는 방법 중 하나는 네이밍을 일관되게 사용하는 데 있다. 관련된 좋은 아티클, [15 Best Practices of Variable & Method Naming](http://codebuild.blogspot.com/2012/02/15-best-practices-of-variable-method.html)을 정리한다.
...
5. 변수에 관련된 회사의 네이밍 규칙을 따르고, 애플리케이션 내에서도 일관된 변수 이름에 쓴다. 예를 들어, txtUserName, lblUserName, cmbSchoolType 등과 같이. 그렇지 않으면, 가독성이 떨어지고 검색/바꾸기 툴 사용 측면에서 사용할 수 없게 된다.
...
9. 변수 이름에 ASCII가 아닌 문자를 사용하지 않는다. 특정 플랫폼에서는 작동할지도 모르지만, 다른 플랫폼에서 작동하지 않을 수 있다.

10. 너무 긴 변수 이름을 사용하지(예로, 50자 길이). 너무 긴 이름은 추잡하고, 읽기 어려운 코드이다. 게다가, 어떤 컴파일러는 최대 길이(character limit)에 의해 작동되지 않는다.

11. 네이밍을 위해 자연 언어를 하나로 정하고 그것을 사용한다. 예를 들어, 영어와 독일어가 혼합된 이름은 비일관적이고 읽기 힘들다.

12. 메소드를 위해서도 의미 있는 네이밍을 사용한다. 이름은 메소드의 정확한 동작(action)을 구체적으로 나타내고, 대부분의 경우 동사로 시작(createPasswordHash 등)한다.

13. 메소드에 대해서도 회사의 네이밍 규칙에 따르고, 애플리케이션 내에서 일관되게 메소드 이름을 쓴다. 예를 들어, getTxtUserName(), getLblUserName(), isStudentApproved() 등이다. 그렇지 않으면, 가독성이 떨어지고, 검색/바꾸기 툴의 사용 측면에서 유효하지 않게 된다.

14. 프로그래밍 언어의 표준에 따라 대/소문자 문자열을 일관되지 않는 상태로 사용하지 않는다. 예를 들어, getUserName, GetUserName, getusername 등의 혼합 말이다.
  - Java 의 예로 올바른 경우는.
