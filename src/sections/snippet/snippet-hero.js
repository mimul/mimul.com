import { Link } from "gatsby";
import React from "react";
import { GatsbyImage } from "gatsby-plugin-image"

const SnippetHero = ({ snippet, authors }) => {
  return (
    <section className="hero bgcolor-25 min-h160 position-relative">
      <div className="padding-1">
        <div className="container">
          <div className="post-caption">
            <p className="text-uppercase ">
              {snippet.topics && snippet.topics.length ? (
                <span className="taglist">
                  {snippet.topics
                    .map((tag) => (
                      <Link
                        key={tag}
                        className="has-text-warning has-text-weight-bold is-uppercase p-category"
                        to={`/topics/${tag}/`}
                      >
                        {tag}
                      </Link>
                    ))
                    .reduce((prev, curr, index) => [
                      prev,
                      <span
                        key={index}
                        className="has-text-primary has-text-weight-light sep"
                      >
                        {" "}
                        |{" "}
                      </span>,
                      curr,
                    ])}
                </span>
              ) : null}
            </p>
            <div className="divider" />
            <h1 className="title is-2 is-light is-semibold is-spaced main-title p-name">
              {snippet.title}
            </h1>
            <h2 className="subtitle p-summary">{snippet.excerpt}</h2>
            {authors && authors.length === 1 ? (
              <div className="author-block">
                <div className="image is-64x64">
                  <GatsbyImage image={authors[0].avatar.small} alt={authors[0].name} />
                </div>
                <div className="author-name">
                  <span>
                    <a
                      className="twitter-follow-button"
                      href={
                        "https://twitter.com/" +
                        authors[0].twitter.replace("@", "")
                      }
                      data-show-screen-name="false"
                    >
                      Follow {authors[0].twitter}
                    </a>
                  </span>
                  <span className="p-author h-card">{authors[0].name}</span>
                  <span>
                    {snippet.date} &middot; {snippet.timeToRead} &middot; Last
                    Updated:{" "}
                    <time
                      dateTime={snippet.lastModifiedTime}
                      className="dt-published"
                    >
                      {snippet.lastModifiedTimeString}
                    </time>
                  </span>
                </div>
              </div>
            ) : null}
          </div>
          {/* <div className="column is-5">
            <figure className="image">
              <Image
                src={article.hero.full}
                className={`blog-featured`}
                alt={article.title}
              />
            </figure>
          </div> */}
        </div>
      </div>
    </section>
  );
};
export default SnippetHero;
